...
import {
    unlockActionsToValue,
    unlockPurseToValue,
    unlockColorsToValue,
    unlockMovementToValue,
    unlockKeysToValue,
    unlockMapMaxValues,
...
export const gameControllerFactory = () => {
    const gameController = {
        ...
        initializeCitiesAndState() {
            logicBundle.playerBoardAndInformationController.initializePlayerInfoBoards(
                this.playerArray,
            );
            logicBundle.turnTrackerController.updateTurnTracker(
                this.playerArray[0],
            );
            this.currentTurn = 0;
            ...
            Object.keys(boardConfig).forEach((cityKey) => {
                ...
                const cityDiv = logicBundle.boardController.createCity({
                    ...city,
                });
                ...
                if (city.neighborRoutes) {
                    city.neighborRoutes.forEach((routeArray) => {
                        ...
                        logicBundle.boardController.createRouteAndTokenFromLocations(
                            {
                                ...
                                element1:
                                    this.cityStorageObject[cityKey].ownElement,
                                element2:
                                    this.cityStorageObject[neighborCityName]
                                        .ownElement,
                                tokenDirection: TOKEN_CONFIG_BY_ROUTES[routeId],
                                isStartingToken:
                                    !!TOKEN_CONFIG_BY_ROUTES[routeId][2],
                                tokenValue,
                            },
                        );
                        ...
                        const addRoutesToCity = (cityName) => {
                            const cityToModify =
                                this.cityStorageObject[cityName];
                        ...
                        for (let i = 0; i < length; i++) {
                            ...
                            this.routeStorageObject[routeId].routeNodes[
                                nodeId
                            ] = {
                                nodeId,
                                occupied: false,
        ...
        advanceTurn(lastPlayer) {
            ...
            if (this.tokensCapturedThisTurn.length > 0) {
                this.tokenPlacementInformation.tokensToPlace =
                    this.tokensCapturedThisTurn.length;
            ...
            if (IS_HOTSEAT_MODE) {
                logicBundle.playerBoardAndInformationController.focusOnPlayerBoard(
                    this.getActivePlayer(),
                    this.playerArray,
                );
            }
        ...
        replaceTokens(player) {
            ...
            const currentReplacement = getRandomArrayElementAndModify(
                this.regularTokensArray,
            );
            this.tokenPlacementInformation.currentReplacement =
                currentReplacement;
            ...
            logicBundle.turnTrackerController.updateTurnTrackerWithTokenInfo(
                player,
                currentReplacement,
                tokensToPlace,
            );
            ...
            logicBundle.boardController.toggleAllTokenLocations(
                Object.keys(this.routeStorageObject),
                'visible',
            );
        ...
        replaceTokenAtLocation(routeId, playerId) {
            const player = this.validatePlayerIsActivePlayer(
                playerId,
                this.getActivePlayer(),
            );
            if (!player) {
            ...
            if (this.routeStorageObject[routeId].token) {
                ...
                logicBundle.inputHandlers.warnInvalidAction(
                    'That location already has a token.',
                );
                return;
            }
            ...
            for (let nodeId in this.routeStorageObject[routeId].routeNodes) {
                if (
                    this.routeStorageObject[routeId].routeNodes[nodeId].occupied
                ) {
                    console.warn('Route must be unoccupied.');
                    logicBundle.inputHandlers.warnInvalidAction(
                        'Route must be unoccupied.',
                    );
                    return;
                }
            }
            ...
            if (!citiesHaveFreeSpot) {
                console.warn(
                    'City endpoints must have at least one open spot among them.',
                );
                logicBundle.inputHandlers.warnInvalidAction(
                    'City endpoints must have at least one open spot among them.',
                );
            ...
            logicBundle.boardController.addTokenToRoute(
                routeId,
                this.tokenPlacementInformation.currentReplacement,
                'silver',
            );
            // 7. We also need to update the route storage object
            this.routeStorageObject[routeId].token =
                this.tokenPlacementInformation.currentReplacement;
            ...
            logicBundle.playerBoardAndInformationController.componentBuilders.updateTokenTracker(
                player,
                this.tokenPlacementInformation.tokensToPlace,
            );
            // 8. Should game log that a new token was placed
            logicBundle.logController.addTextToGameLog(
                `$PLAYER1_NAME placed a ${this.tokenPlacementInformation.currentReplacement}
            token at ${routeId}.`,
                player,
            );
            ...
            if (this.tokenPlacementInformation.tokensToPlace > 0) {
                ...
                console.log(
                    'There are more tokens to place, repeating the process',
                );
                return;
            } else {
        ...
        resolveAction(player) {
            ...
            this.playerArray.forEach((player) => {
                logicBundle.playerBoardAndInformationController.componentBuilders.updateSupplyAndBank(
                    player,
                );
            });
        ...
        placeWorkerOnNodeAction(nodeId, shape, playerId) {
            ...
            const playerShapeKey =
                shape === 'square' ? 'supplySquares' : 'supplyCircles';
            if (player[playerShapeKey] < 1) {
                console.warn(`Not enough ${shape}s in your supply`);
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    `Not enough ${shape}s in your supply!`,
                );
                return;
            }
            ...
            if (
                ...
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    'That route node is already occupied!',
                );
                return;
            }
            ...
            logicBundle.logController.addTextToGameLog(
                `$PLAYER1_NAME placed a ${shape} on ${nodeId}`,
                player,
            );
        ...
        placePieceOnNode(nodeId, shape, player) {
            ...
            logicBundle.boardController.addPieceToRouteNode(
                nodeId,
                player.color,
                shape,
            );
        ...
        selectPieceToMove(nodeId, playerId) {
            ...
            const routeId = getRouteIdFromNodeId(nodeId);
            const node =
                gameController.routeStorageObject[routeId].routeNodes[nodeId];

            if (!node.occupied || node.playerId !== playerId) {
                console.warn('You do not have a piece on this route node.');
                logicBundle.inputHandlers.warnInvalidAction(
                    'You do not have a piece on this route node.',
                );
                return;
            } else {
                ...
                logicBundle.inputHandlers.additionalInfo =
                    'selectLocationToMoveTo';
                logicBundle.inputHandlers.updateActionInfoText(
                    `You have a selected a ${node.shape}. Select an unoccupied route node to move there.`,
                );
        ...
        movePieceToLocation(nodeId, playerId) {
            ...
            if (targetNode.occupied) {
                ...
                logicBundle.inputHandlers.warnInvalidAction(
                    'This route node is already occupied.',
                );
                return;
            }
            this.placePieceOnNode(nodeId, shape, player);
            ...
            logicBundle.logController.addTextToGameLog(
                `$PLAYER1_NAME moved a ${shape} from ${originNode.nodeId} to ${nodeId}`,
                player,
            );
            const clearedProps = {
                occupied: false,
            ...
            logicBundle.boardController.clearPieceFromRouteNode(
                originNode.nodeId,
            );
            gameController.moveInformation.movesUsed++;
            if (
                gameController.moveInformation.movesUsed === player.maxMovement
            ) {
            ...
            logicBundle.inputHandlers.updateActionInfoText(
                `Select one of your own pieces to move. You have ${
                    player.maxMovement -
                    gameController.moveInformation.movesUsed
                } left.`,
            );
        ...
        endMoveAction(playerId) {
            ...
            if (!gameController.moveInformation.movesUsed) {
                ...
                logicBundle.logController.addTextToGameLog(
                    `$PLAYER1_NAME moved ${this.moveInformation.movesUsed} pieces.`,
                    player,
                );
                this.resolveAction(player);
            }
        ...
        resupply(playerId) {
            ...
            if (player.bankedCircles === 0 && player.bankedSquares === 0) {
                ...
                logicBundle.inputHandlers.warnInvalidAction(
                    'There is nothing in your bank to resupply with.',
                );
                return;
            }
            ...
            logicBundle.logController.addTextToGameLog(
                `$PLAYER1_NAME resupplied ${resuppliedCircles} circles and ${resuppliedSquares} squares.`,
                player,
            );
            this.resolveAction(player);
        ...
        bumpPieceFromNode(nodeId, shape, playerId) {
            ...
            if (!node.occupied || node.playerId === playerId) {
                console.warn(
                    'The route node needs to be occupied by a rival player.',
                );
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    'The route node needs to be occupied by a rival player.',
            ...
            if (
                ...
                console.warn(
                    `You need at least ${squareCost} squares and ${circleCost} circles in your supply`,
                );
            ...
            this.bumpInformation.bumpedPlayer =
                this.playerArray[bumpedPlayerId];
        ...
        placeBumpedPieceOnNode(nodeId, shape, playerId) {
            ...
            const isValidNode = this.checkThatLocationIsAdjacent(
                this.bumpInformation.bumpedLocation,
                nodeId,
            );
            if (!isValidNode) {
                console.warn('Selected node must be part of an adjacent route');
                logicBundle.inputHandlers.warnInvalidAction(
                    'Selected node must be part of an adjacent route',
                );
                return;
            }
            ...
            if (
                shape === 'circle' &&
                this.bumpInformation.circlesToPlace === 0
            ) {
                console.warn('You cannot place another circle.');
                logicBundle.inputHandlers.warnInvalidAction(
                    'You cannot place another circle.',
                );
                return;
            }
            if (
                shape === 'square' &&
                this.bumpInformation.squaresToPlace === 0
            ) {
                console.warn('You cannot place another square.');
                logicBundle.inputHandlers.warnInvalidAction(
                    'You cannot place another square.',
                );
                return;
            }
            // 3. Check if the player has used their free shape - if so clear it
            if (
                this.bumpInformation.freePiece &&
                ...
                if (player.bankedSquares === 0) {
                    if (player.supplySquares === 0) {
                        ...
                        console.error(
                            'You have no squares in your bank or supply.',
                        );
                        logicBundle.inputHandlers.warnInvalidAction(
                            'You have no squares in your bank or supply.',
                        );
            ...
            const outOfMoves =
                ...
                0;
            const outOfPieces =
                !this.bumpInformation.free &&
                player.bankedSquares + player.supplySquares === 0;

            if (outOfMoves || outOfPieces) {
                logicBundle.logController.addTextToGameLog(
                    `$PLAYER1_NAME displaced $PLAYER2_NAME at ${nodeId}`,
                    this.bumpInformation.bumpingPlayer,
                    player,
                );
            ...
            logicBundle.inputHandlers.setUpBumpActionInfo(
                nodeId,
                this.bumpInformation.bumpedShape,
                this.bumpInformation.squaresToPlace,
                this.bumpInformation.circlesToPlace,
            );
            logicBundle.turnTrackerController.updateTurnTrackerWithBumpInfo({
        ...
        checkThatLocationIsAdjacent(bumpedNodeId, targetNodeId) {
            ...
            while (true) {
                ...
                if (failSafe > 15) {
                    console.error(
                        'Infinite loop in checkThatLocationIsAdjacent, breaking out',
                    );
                    return;
                }
                ...
                for (let routeId of routesToChecks) {
                    ...
                    if (routeId === targetRouteId) {
                        console.warn(
                            `Found a match after ${failSafe} completed loops`,
                        );
                        return true;
                    }
                    ...
                    route.cities.forEach((cityName) => {
                        ...
                        city.routes.forEach((innerRouteId) => {
                            if (
                                !routesToChecksNext.includes(innerRouteId) &&
                                innerRouteId !== routeId
                            ) {
                ...
                if (hadAnUnoccupiedNode) {
                    console.warn(
                        `Found a hadAnUnoccupiedNode after ${failSafe} completed loops`,
                    );
                    return false;
                }
                failSafe++;
        ...
        captureCity(cityName, playerId) {
            ...
            const routeCheckOutcome = this.checkIfPlayerControlsARoute(
                playerId,
                cityName,
            );
            ...
            if (!routeCheckOutcome) {
                ...
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    'You do not have a completed route',
                );
                return;
            }
            if (player.currentActions === 0) {
                ...
                console.error(
                    "You don't have enough actions, how did you even get here? The turn was supposed to advance!",
                );
                return;
            }
            if (city.openSpotIndex === city.spotArray.length) {
                ...
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    `The city of ${city.cityName} is already full.`,
                );
                return;
            }
            const [targetShape, targetColor] =
                city.spotArray[city.openSpotIndex];
            if (!player.unlockedColors.includes(targetColor)) {
                console.warn(`You haven't unlocked ${targetColor}.`);
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    `You haven't unlocked ${targetColor}.`,
                );
                return;
            }
            if (routeCheckOutcome[targetShape] === 0) {
                ...
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    `You don't have a ${targetShape} in your route.`,
                );
                return;
            }
            if (city.freePoint) {
                ...
                logicBundle.logController.addTextToGameLog(
                    `$PLAYER1_NAME collected the free point at ${cityName}`,
                    player,
                );
            }
            ...
            if (this.tokenUsageInformation.tokenAction === 'bonusPost') {
                console.warn(
                    `Trying to capture ${cityName} with an additional post`,
                );
                let usedShape;
                if (player.bankedSquares > 0) {
                    usedShape = 'square';
                ...
                gameController.cityStorageObject[
                    cityName
                ].bonusSpotOccupantArray.push([playerId, usedShape]);
                // We subtract one from the length as we're including itself
                logicBundle.boardController.addBonusPieceToCity(
                    city,
                    player.color,
                    usedShape,
            ...
            logicBundle.logController.addTextToGameLog(
                `$PLAYER1_NAME captured the city of ${cityName}.`,
                player,
            );
            this.resolveAction(player);
        },
        upgradeAtCity(cityName, playerId) {
            ...
            if (!city.unlock) {
                console.warn(
                    `The city of ${city.cityName} doesn't have a corresponding unlock.`,
                );
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    `The city of ${city.cityName} doesn't have a corresponding unlock.`,
            ...
            if (!routeCheckOutcome) {
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    'You cannot upgrade without a completed route.',
                );
                return;
            }
        ...
        performUnlock(player, unlock) {
            const noFurtherUpgrades = (unlockName) => {
                console.warn(
                    `You can't upgrade your ${unlockName} any further.`,
                );
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.warnInvalidAction(
                    `You can't upgrade your ${unlockName} any further.`,
            ...
            switch (unlock) {
                case 'purse':
                    ...
                    player.unlockArrayIndex.purse++;
                    player.purse =
                        unlockPurseToValue[player.unlockArrayIndex.purse];
                    logicBundle.logController.addTextToGameLog(
                        `$PLAYER1_NAME has upgraded their resupply. They now have ${player.purse}.`,
                        player,
                    );
                    logicBundle.playerBoardAndInformationController.unlockPieceFromBoard(
                        player,
                        player.unlockArrayIndex.purse,
                        unlock,
                    );
                ...
                case 'actions': {
                    ...
                    player.unlockArrayIndex.actions++;
                    player.maxActions =
                        unlockActionsToValue[player.unlockArrayIndex.actions];
                    ...
                    if ([1, 3, 5].includes(player.unlockArrayIndex.actions)) {
                        ...
                        actionUpgradeText +=
                            ' They get a free action as a result';
                        logicBundle.turnTrackerController.updateTurnTracker(
                            player,
                        );
                    }
                ...
                case 'colors':
                    if (
                        ...
                        unlockColorsToValue.length - 1
                    ) {
                        noFurtherUpgrades('available colors');
                        return false;
                    }
                    player.unlockArrayIndex.colors++;
                    player.unlockedColors.push(
                        unlockColorsToValue[player.unlockArrayIndex.colors],
                    );
                    logicBundle.logController.addTextToGameLog(
                        `$PLAYER1_NAME has upgraded their available colors. They can now place pieces on ${player.unlockedColors.slice(
                            -1,
                        )}.`,
                        player,
                    );
                ...
                case 'maxMovement':
                    if (
                        ...
                        unlockMovementToValue.length - 1
                    ) {
                    ...
                    logicBundle.logController.addTextToGameLog(
                        `$PLAYER1_NAME has upgraded their maximum movement. They now have ${player.maxMovement}.`,
                        player,
                    );
                    logicBundle.playerBoardAndInformationController.unlockPieceFromBoard(
                        player,
                ...
                case 'keys':
                    ...
                    player.unlockArrayIndex.keys++;
                    player.keys =
                        unlockKeysToValue[player.unlockArrayIndex.keys];
                    logicBundle.logController.addTextToGameLog(
                        `$PLAYER1_NAME has upgraded their route multiplier. They now have ${player.keys}.`,
                        player,
                    );
                    logicBundle.playerBoardAndInformationController.unlockPieceFromBoard(
                        player,
            ...
            if (unlock === 'maxMovement') {
                logicBundle.logController.addTextToGameLog(
                    `$PLAYER1_NAME has unlocked a circle for their supply.`,
                    player,
                );
                player.supplyCircles++;
            } else {
                logicBundle.logController.addTextToGameLog(
                    `$PLAYER1_NAME has unlocked a square for their supply.`,
                    player,
                );
                player.supplySquares++;
            }
        ...
        checkIfPlayerControlsARoute(playerId, cityName) {
            ...
            for (const routeId in this.routeStorageObject) {
                if (
                    this.routeStorageObject[routeId].cities.includes(cityName)
                ) {
                    let isComplete = true;
                    let square = 0;
                    let circle = 0;
                    for (const nodeId in this.routeStorageObject[routeId]
                        .routeNodes) {
                        const node =
                            this.routeStorageObject[routeId].routeNodes[nodeId];
        ...
        routeCompleted(routeId, player) {
            logicBundle.logController.addTextToGameLog(
                `$PLAYER1_NAME has completed route ${routeId}`,
                player,
            );
            ...
            if (route.token) {
                ...
                logicBundle.logController.addTextToGameLog(
                    `$PLAYER1_NAME has claimed a ${tokenKind} token.`,
                    player,
                );
                player.currentTokens.push(tokenKind);
                logicBundle.playerBoardAndInformationController.componentBuilders.updateTokensInSupplyAndBank(
                    player,
                );
                logicBundle.playerBoardAndInformationController.componentBuilders.updateTokenTracker(
                    player,
            ...
            route.cities.forEach((cityId) => {
                const controller = this.calculateControllingPlayer(
                    this.cityStorageObject[cityId],
                );
                if (controller) {
                    this.scorePoints(1, controller);
                }
            });
            for (const nodeToClearId in this.routeStorageObject[routeId]
                .routeNodes) {
        ...
        handleTokenMenuRequest(playerId) {
            ...
            if (player.currentTokens.length === 0) {
                ...
                logicBundle.inputHandlers.warnInvalidAction(
                    "You don't have any tokens to use.",
                );
                return;
            }
        ...
        finishTokenUsage(player, tokenType) {
            logicBundle.logController.addTextToGameLog(
                `$PLAYER1_NAME used a ${TOKEN_READABLE_NAMES[tokenType]} token.`,
                player,
            );
            logicBundle.inputHandlers.clearAllActionSelection();
        ...
        tokenActions: {
            gainActions(player, actionsNumber) {
                console.warn(
                    `${player.name} is gaining ${actionsNumber} actions`,
                );
                player.currentActions += actionsNumber;
                logicBundle.turnTrackerController.updateTurnTracker(player);
                gameController.finishTokenUsage(
                    player,
                    actionsNumber === 3 ? 'threeActions' : 'fourActions',
                );
            },
            freeUpgradeSetup(player) {
                ...
                for (let unlockKey in player.unlockArrayIndex) {
                    if (
                        player.unlockArrayIndex[unlockKey] <
                        unlockMapMaxValues[unlockKey] - 1
                    ) {
                ...
                if (availableUpgrades.length === 0) {
                    logicBundle.inputHandlers.clearAllActionSelection();
                    console.warn(
                        "You've already maxed out all upgrades. Well done!",
                    );
                    logicBundle.inputHandlers.warnInvalidAction(
                        "You've already maxed out all upgrades. Well done!",
                    );
                ...
                logicBundle.inputHandlers.populateUpgradeMenuFromToken(
                    availableUpgrades,
                );
            },
            useFreeUpgrade(upgradeType, playerId) {
                const player = gameController.validatePlayerIsActivePlayer(
                    playerId,
            ...
            switchPost() {
                logicBundle.inputHandlers.clearAllActionSelection();
                logicBundle.inputHandlers.selectedAction =
                    'switchPostSelection';
                logicBundle.inputHandlers.updateActionInfoText(
                    'Select two spots in the same city to exchange. You must own one of them.',
                );
            },
            selectedPostToSwitch(cityId, citySpotNumber, playerId) {
                ...
                if (!(playerOwns && rivalOwns)) {
                    console.warn(
                        `The city of ${cityId} needs to have a post owned by you and a post owned by a rival to switch.`,
                    );
                    logicBundle.inputHandlers.warnInvalidAction(
                        `The city of ${cityId} needs to have a post owned by you and a post owned by a rival to switch.`,
                    );
                ...
                if (!gameController.tokenUsageInformation.switchSpot) {
                    ...
                    gameController.tokenUsageInformation.switchSpot = [
                        cityId,
                        citySpotNumber,
                    ];
                    logicBundle.inputHandlers.updateActionInfoText(
                        `\nYou selected post number ${citySpotNumber} in ${cityId}. Select one more spot in ${cityId} to exchange posts.`,
                        false,
                    );
                    return;
                }
                ...
                const previousCityId =
                    gameController.tokenUsageInformation.switchSpot[0];
                const previousSpotNumber =
                    gameController.tokenUsageInformation.switchSpot[1];

                // 1. Check that it's the same city
                // 2. Check that one is you and the other is a different player
                if (cityId !== previousCityId) {
                    console.warn(
                        'Both trading posts need to be in the same city.',
                    );
                    logicBundle.inputHandlers.warnInvalidAction(
                        'Both trading posts need to be in the same city.',
                    );
                ...
                const occupantOne =
                    gameController.cityStorageObject[previousCityId].occupants[
                        previousSpotNumber
                    ];
                const occupantTwo =
                    gameController.cityStorageObject[cityId].occupants[
                        citySpotNumber
                    ];
                const haveDifferentOwners = occupantOne !== occupantTwo;
                const oneIsOwnedByPlayer =
                    occupantOne === playerId || occupantTwo === playerId;

                if (!(haveDifferentOwners && oneIsOwnedByPlayer)) {
                    console.warn(
                        'Both trading posts must be owned by different players, one of whom is you.',
                    );
                    logicBundle.inputHandlers.warnInvalidAction(
                        'Both trading posts must be owned by different players, one of whom is you.',
                    );
                ...
                logicBundle.boardController.switchPieceColor(
                    `piece-${previousCityId}-${previousSpotNumber}`,
                    `piece-${cityId}-${citySpotNumber}`,
                );
            ...
            bonusPost() {
                ...
                logicBundle.inputHandlers.updateActionInfoText(
                    'Select a city to capture. You will receive a bonus trading post.',
                );
            },
            moveThree() {
                logicBundle.inputHandlers.clearAllActionSelection();
                ...
                logicBundle.inputHandlers.updateActionInfoText(
                    'Select an opposing piece and a location to move it to. You can do this three times',
                );
            ...
            selectMoveThreePiece(nodeId, playerId) {
                ...
                const routeId = getRouteIdFromNodeId(nodeId);
                const node =
                    gameController.routeStorageObject[routeId].routeNodes[
                        nodeId
                    ];
                // 1. Validate that the location is occupied and owned by an opponent - warn and return if not
                if (!node.occupied || node.playerId === playerId) {
                    console.warn(
                        'You must select a node occupied by an opposing piece.',
                    );
                    logicBundle.inputHandlers.warnInvalidAction(
                        ' You must select a node occupied by an opposing piece.',
                    );
                ...
                logicBundle.inputHandlers.updateActionInfoText(
                    `You selected the piece at ${nodeId}. Select an empty spot to move it to.`,
                );
            ...
            selectMoveThreeLocation(nodeId, playerId) {
                ...
                const originId =
                    gameController.tokenUsageInformation.originLocation;
                const routeOriginId = getRouteIdFromNodeId(originId);
                const originNode =
                    gameController.routeStorageObject[routeOriginId].routeNodes[
                        originId
                    ];
                const originPlayer =
                    gameController.playerArray[originNode.playerId];
                ...
                gameController.placePieceOnNode(
                    nodeId,
                    originShape,
                    originPlayer,
                );
                ...
                logicBundle.inputHandlers.populateMoveThreeMenu(
                    gameController.tokenUsageInformation.movesLeft,
                );
                logicBundle.inputHandlers.additionalInfo = 'selectPiece';
        ...
        endGame() {
            // TODO
            console.warn(
                'The game ended but I have not implemented end game point calculations yet. Sorry.',
            );
        },
        ...
        saveGame() {
            ...
            window.localStorage.setItem(
                'regularTokensArray',
                JSON.stringify(this.regularTokensArray),
            );
            window.localStorage.setItem(
                'playerArray',
                JSON.stringify(this.playerArray),
            );
            window.localStorage.setItem(
                'cityStorageObject',
                JSON.stringify(this.cityStorageObject),
            );
            window.localStorage.setItem(
                'routeStorageObject',
                JSON.stringify(this.routeStorageObject),
            );
        },
        loadGame() {
            this.playerArray = JSON.parse(
                window.localStorage.getItem('playerArray'),
            );
            this.initializeCitiesAndState();
            this.cityStorageObject = JSON.parse(
                window.localStorage.getItem('cityStorageObject'),
            );
            this.routeStorageObject = JSON.parse(
                window.localStorage.getItem('routeStorageObject'),
            );
            this.regularTokensArray = JSON.parse(
                window.localStorage.getItem('regularTokensArray'),
            );
            ...
            Object.keys(this.cityStorageObject).forEach((cityName) => {
                ...
                city.occupants.forEach((id) => {
                    logicBundle.boardController.addPieceToCity(
                        city,
                        this.playerArray[id].color,
                    );
                    city.openSpotIndex++;
                });
                city.bonusSpotOccupantArray.forEach((idAndShape, index) => {
                    logicBundle.boardController.addBonusPieceToCity(
                        city,
                        this.playerArray[idAndShape[0]].color,
                        idAndShape[1],
                        index,
                    );
                });
            ...
            this.playerArray.forEach((player) => {
                Object.keys(player.unlockArrayIndex).forEach((unlockKey) => {
                    if (player.unlockArrayIndex[unlockKey] > 0) {
                        for (
                            let index = 1;
                            index <= player.unlockArrayIndex[unlockKey];
                            index++
                        ) {
                            logicBundle.playerBoardAndInformationController.unlockPieceFromBoard(
                                player,
